...
cmd:option('-rl_env_model_from', 'checkpoints/model_visual7w_telling_gpu.t7', 'path to model for policy gradient reward environment')
...
print('Start load reinforcement learning env model')
assert(string.len(opt.rl_env_model_from) > 0, 'error: must provide a model')
local rl_env_checkpont = torch.load(opt.rl_env_model_from)
local rl_env_modules = rl_env_checkpont.modules
local rl_env = {}
rl_env['rnn'] = rl_env_modules.rnn
rl_env['cnn'] = rl_env_modules.cnn
rl_env['crit'] = nn.QACriterion()
if gpu_mode then
  for k,v in pairs(rl_env) do v:cuda() end
end
rl_env.rnn:createClones()
rl_env.cnn:evaluate()
rl_env.rnn:evaluate()
local qa_pg_crit = nn.QAPGCriterion(rl_env)
rl_env_modules = nil
rl_env_checkpont = nil
if gpu_mode then
  qa_pg_crit:cuda()
end
collectgarbage() -- free some memory
print('End load reinforcement learning env model')
...
local function lossFun()
  ...
  -- split question and gold answer
  local q_lens, a_lens = batch.question_length, batch.answer_length
  local question_labels, answer_labels = utils.split_question_answer(batch.labels, q_lens, a_lens)
  -- make samples
  local opt_sample = {}
  opt_sample['sample_max'] = 3  -- for non-greddy sample
  -- local logprobs = modules.rnn:forward{image_encodings, conv_feat_maps, batch.labels}
  local sample_labels, seq_logprobs, seq_logprobs_pertime = modules.rnn:sample(image_encodings, conv_feat_maps, question_labels, q_lens, opt_sample)
  local samples = net_utils.decode_sequence(vocab, sample_labels)
  local sample_lens = utils.get_sequences_token_cnt(samples)
  -- pack question and sample
  local question_sample_labels = utils.pack_question_answer(question_labels, q_lens, sample_labels, sample_lens, batch.labels:size(1))
  local sample_per_sum_logp = utils.cal_answer_sum_logp(seq_logprobs, sample_lens, gpu_mode)
  -- forward to loss
  -- local loss = modules.crit:forward(logprobs, {batch.labels, batch.question_length})
  local loss = qa_pg_crit:forward({batch.images, question_sample_labels, q_lens, sample_per_sum_logp}, {})
  ...
  local dlogprobs = qa_pg_crit:backward({seq_logprobs_pertime, question_sample_labels, q_lens}, {})
  -- backprop language model
  --local dfc, dconv, _ = unpack(modules.rnn:backward({image_encodings, conv_feat_maps, batch.labels}, dlogprobs))
  local dfc, dconv, _ = unpack(modules.rnn:backward({image_encodings, conv_feat_maps, question_sample_labels}, dlogprobs))
...
while opt.max_iters < 0 or iter <= opt.max_iters do
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    local checkpoint_path = path.join(opt.checkpoint_path, 'policy_gradient_no_baseline_' .. opt.suffix)
  ...
  if losses.total_loss > loss_init * 50 then
